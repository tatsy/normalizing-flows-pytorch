...
class WeightNormLinear(nn.Module):
    def __init__(self, in_dim, out_dim, bias=True, weight_norm=True, scale=False):
...
class SpectralNormLinear(nn.Module):
    def __init__(self, in_dim, out_dim, bias=True, spectral_norm=True):
        super(SpectralNormLinear, self).__init__()

        if spectral_norm:
            self.conv = nn.utils.spectral_norm(nn.Linear(in_dim, out_dim, bias=bias))
        else:
            self.conv = nn.Linear(in_dim, out_dim, bias=bias)
...
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.net = nn.Sequential(nn.BatchNorm1d(in_channels), nn.ReLU(inplace=True),
                                 WeightNormLinear(in_channels, out_channels), nn.BatchNorm1d(out_channels),
                                 nn.ReLU(inplace=True), WeightNormLinear(out_channels, out_channels))

        if in_channels != out_channels:
            self.bridge = nn.Sequential(WeightNormLinear(in_channels, out_channels), )
        else:
...
class Network(nn.Module):
    def __init__(self, in_channels, out_channels, base_filters=32):
        ...
        self.mid_block = nn.Sequential(ResBlock(base_filters, base_filters), ResBlock(base_filters, base_filters))

        self.out_block = nn.Sequential(nn.BatchNorm1d(base_filters), nn.ReLU(inplace=True),
