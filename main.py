...
def main():
    ...
    parser.add_argument(
        "-B", "--batchsize", type=int, default=1024, help="minibatch size"
    )
    parser.add_argument(
        "-N", "--n_samples", type=int, default=1024, help="#samples to be drawn"
    )
    parser.add_argument(
        "--dist_name",
        default="moons",
    ...
    for epoch in range(args.epochs):
        ...
        for i in pbar:
            ...
            pbar.set_description(
                "epoch #{:d}: loss={:.5f}".format(epoch + 1, loss.item())
            )
